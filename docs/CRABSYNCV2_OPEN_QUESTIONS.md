...
- Keys must not sync.
- Keys unlock permanent/new content and remain local-only progression currency, not shared run resources.
- Runtime evidence may still mention key visibility, but key visibility does not imply sync eligibility.
...
| Can a safe P2P carrier be proven for CrabSyncV2 state? | External relay/server is not the only possible architecture if a proven player/health/resource sharing path can carry extra state safely. | Hypothesis only; no safe carrier or payload model is proven. | P2P carrier feasibility research after local metadata proof and proven player/resource/health paths. | Whether inventory/resource state can be shared through P2P piggybacking or whether relay/server remains required. | High |
