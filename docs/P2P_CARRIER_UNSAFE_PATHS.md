...
| carrier-forbidden-keys | `CrabPS` or resource state | Keys as custom carrier | gameplay-authoritative; currency/progression | Carrier research plan forbidden list; key state affects gameplay/progression. | forbidden-template | Keys policy remains unresolved for sync and cannot carry payloads. |
