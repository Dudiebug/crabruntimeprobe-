...
| `p2p-health-convergence-read` | Verify whether peer-visible health surfaces are sufficient for deterministic convergence planning using [CrabSyncV2 Health P2P Model](CRABSYNCV2_HEALTH_P2P_MODEL.md). | `allowHealthProbes = true`; no unscoped `CrabHC`; writes/RPC false. | PlayerState-scoped health sampling across lifecycle contexts. | Unscoped `CrabHC`, writes, mutating RPCs. | `access_evidence_<session>.jsonl`. | Stable read snapshots by role/context with convergence inputs captured. | Crash or evidence of unsafe source path drift. | Health convergence model inputs. |
| `p2p-resource-convergence-read` | Verify deterministic convergence inputs for equipment/crystals/slots using [CrabSyncV2 Resource P2P Model](CRABSYNCV2_RESOURCE_P2P_MODEL.md). | Resource/equipment read gates true; writes/RPC false. | Capped cross-player reads for proven resource/equipment paths. | Writes, mutating RPCs, custom payload injection. | `access_evidence_<session>.jsonl`. | Stable peer-visible snapshots and range-clamp validation inputs. | Crash, stale identity mismatch, or unsafe path regression. | Resource convergence planning. |
